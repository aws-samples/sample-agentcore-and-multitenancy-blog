...
# ----- 2c. Zip FHIR Lambda code with dependencies -----
FHIR_LAMBDA_ZIP_FILE="fhir_lambda.zip"
echo "📦 Packaging FHIR Lambda with dependencies into $FHIR_LAMBDA_ZIP_FILE..."
if [ -f "prerequisite/lambda/python/fhir_lambda.py" ]; then
  ...
  cp prerequisite/lambda/python/fhir_lambda.py fhir_lambda_package/
  cd fhir_lambda_package
  zip -r "../$FHIR_LAMBDA_ZIP_FILE" . > /dev/null
  cd - > /dev/null
  rm -rf fhir_lambda_package
else
  echo "⚠️  FHIR Lambda code not found, skipping..."
fi
...
if [ -f "$FHIR_LAMBDA_ZIP_FILE" ]; then
  ...
  if [ $fhir_exit_code -eq 0 ]; then
    echo "✅ FHIR API Gateway deployed successfully"
  fi
else
