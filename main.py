...
from agent_config.jwt_utils import extract_tenant_id_from_jwt
...
async def invoke(payload, context):
    ...
    if not CustomerSupportContext.get_gateway_token_ctx():
        ...
        # Get tenant_id from payload (sent by API Gateway Lambda)
        tenant_id = payload.get('tenant_id', 'basic')
        print(f"🔍 DEBUG: Got tenant_id from payload: {tenant_id}")
