...
class CustomerSupportContext:
    """
    Enhanced Context Manager for Healthcare Multi-Tenant Clinical Document Assistant (Premium Tier).
    ...
    def get_actor_id_ctx(cls) -> Optional[str]:
        ...
        try:
            ...
            tier = cls.get_tenant_id_ctx() or 'premium'
            clinic = cls.get_clinic_id_ctx() or 'demo-hospital'
    ...
    def get_tenant_key_ctx(cls) -> Optional[str]:
        """Get combined tier-clinic key for routing (e.g., 'premium-hospital-a')"""
        if cls._tenant_key:
    ...
    def get_s3_prefix_ctx(cls) -> Optional[str]:
        """Get S3 document prefix for clinic isolation (e.g., 'premium-tier/hospital-a/')"""
        if cls._s3_prefix:
    ...
    def get_memory_id_ctx(cls) -> Optional[str]:
        """Get memory resource identifier (e.g., 'healthcare-premium-memory')"""
        if cls._memory_id:
